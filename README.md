...
I built this project to see if an AI "Manager" could outperform individual trading strategies by dynamically switching between them. Instead of relying on just one algorithm, I created a system where a manager agent watches the market and decides how much mnoney to give to three different specialist agents (Surfer, Sniper, and Sentinel) based on who is performing best.

## How it works (The Architecture)
* The Specialists: I trained three separate PPO agents using 'Stable Baselines3'. Each has a different "personality":
    * Surfer: Loves trends .
    * Sniper: More aggressive, higher entropy (more risks).
    * Sentinel: Conservative, focuses on not losing money. Usually goes into treasury bonds
...
* Ablation Studies
    ...
    * Goal: Prove that the total team (manager + specialists) creates a smoother equity curve than just betting it all on the aggressive "Sniper" agent.
* Overfitting Check (Walk-Forward Validation):
    ...
    * This forces the model to constantly adapt to unseen data (like the 2020 crash or 2022 bear market) rather than memorizing history.
